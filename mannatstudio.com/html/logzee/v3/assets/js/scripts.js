...
$(document).ready(function () {
  $(".numinate").appear(); // activate appear listener

  $(document.body).on("appear", ".numinate", function () {
    var $el = $(this);

    // Prevent running twice
    if ($el.hasClass("numinate-started")) return;
    $el.addClass("numinate-started");

    $el.numinate({
      from: $el.data("from") || 0,
      to: $el.data("to") || 0,
      runningInterval: 2000, // total animation duration (ms)
      stepUnit: 1,
      format: "%counter%"
    });
  });
